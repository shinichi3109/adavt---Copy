...
def grayscale():
    img = Image.open("static/img/img_now.png")
    ...
    new_img.save("static/img/img_now.png")
...
def edge_detection():
    ...
    f = open("static/img/img_now.png", "rb")
    data = f.read()
    ...
    f.close()
    f = open("static/img/img_now.png", "wb")
    f.write(img)
    f.close()
...
def histogram_rgb():
    img_path = "static/img/img_now.png"
    ...
    if is_grey_scale(img_path):
        ...
        plt.savefig(f'static/img/grey_histogram.png', dpi=300)
        plt.clf()
    else:
        r = img_arr[:, :, 0].flatten()
        g = img_arr[:, :, 1].flatten()
        ...
        for data in data_hist:
            ...
            plt.savefig(f'static/img/{data[0]}_histogram.png', dpi=300)
            plt.clf()
...
def histogram_equalizer():
    img = cv2.imread('static\img\img_now.png', 0)
    my_cdf = cdf(df(img))
    # use linear interpolation of cdf to find new pixel values. Scipy alternative exists
    image_equalized = np.interp(img, range(0, 256), my_cdf)
    cv2.imwrite('static/img/img_now.png', image_equalized)
